...
try:
    ...
    print("Could not import DBusQtMainLoop, is package "
          "'python-dbus.mainloop.pyqt5' installed?")
...
def init(app_name):
    ...
    if mainloop is not None:
        ...
        DBUS_IFACE.connect_to_signal('NotificationClosed',
...
class Notification(object):
    ...
    def __init__(self, title, body='', icon='', timeout=-1):
        ...
        Args:
            ...
            timeout (TYPE, optional): The time in ms before the notification
                                      hides, -1 for default, 0 for never
        """
        ...
        self.timeout = timeout          # time in ms before disappearing
    ...
    def show(self):
        if DBUS_IFACE is None:
            raise UninitializedError("You must call 'notify.init()' before "
                                     "'notify.show()'")
    ...
    def addAction(self, action, label, callback, user_data=None):
        ...
        Args:
            ...
            label (str):                The text to display on action button
            callback (bound method):    The method to call when the action
                                        is activated
            user_data (any, optional):  Any user data to be passed to the
                                        action callback
        """
    ...
    def _makeActionsList(self):
        ...
        arr = []
        for action, (label, _, _) in self.actions.items():
            arr.append(action)
            arr.append(label)
        return arr
    ...
    def _onActionInvoked(self, action):
        ...
        try:
            _, callback, user_data = self.actions[action]
        except KeyError:
            return
